...
def billProductView(request,productid):
  if 'user_id' in request.session:
    ...
    username=request.session.get('name')
    user=models.UserModelClass.objects.filter(id=user_id)[0]
    categories=models.CategoryModelClass.objects.all()
    ...
    deliverycharges=50
    finaltotal=total+GST+deliverycharges
    return render(request,"billproduct.html",{'products':products,'categories':categories,'username':username,'GST':GST,'deliverycharges':deliverycharges,'finaltotal':finaltotal,'total':total,'productsid':productid})
